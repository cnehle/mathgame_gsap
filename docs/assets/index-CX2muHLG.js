...
`)}};function hQ(e){return new Promise(t=>setTimeout(t,e))}var gQ=class{version;objSvg;morphSvg;panel;runBtn;closeBtn;progressEl;progressStage;progressBar;resultsEl;constructor(e,t,n){this.version=e,this.objSvg=t,this.morphSvg=n,this.panel=_Q(`#perf-panel`),this.runBtn=_Q(`#perf-run-btn`),this.closeBtn=_Q(`#perf-close`),this.progressEl=_Q(`#perf-progress`),this.progressStage=_Q(`#perf-progress-stage`),this.progressBar=_Q(`#perf-progress-bar`),this.resultsEl=_Q(`#perf-results`),this.bindEvents()}bindEvents(){this.runBtn.addEventListener(`click`,()=>this.runTests()),this.closeBtn.addEventListener(`click`,()=>this.hide()),window.addEventListener(`keydown`,e=>{(e.ctrlKey||e.metaKey)&&e.key.toLowerCase()===`p`&&(e.preventDefault(),this.toggle())})}toggle(){this.panel.classList.contains(`hidden`)?this.show():this.hide()}show(){this.panel.classList.remove(`hidden`)}hide(){this.panel.classList.add(`hidden`)}async runTests(){this.runBtn.disabled=!0,this.resultsEl.textContent=``,this.progressEl.style.display=`block`;let e=await new mQ(this.version).run(this.objSvg,this.morphSvg,(e,t)=>{this.progressStage.textContent=e,this.progressBar.style.width=`${t}%`}),t=mQ.formatReport(e);this.resultsEl.textContent=t,console.log(t);try{await navigator.clipboard.writeText(t),this.progressStage.textContent=`✓ Скопировано в буфер обмена`}catch{this.progressStage.textContent=`Результаты в панели — скопируй вручную`}this.runBtn.disabled=!1}};function _Q(e){let t=document.querySelector(e);if(!t)throw Error(`Not found: ${e}`);return t}var vQ=[`#FFD93D`,`#FF6B9D`,`#FF8C69`,`#C3B1E1`],yQ=[`#8a5c00`,`#8a1040`,`#a03010`,`#5a3e8a`],bQ=[`Правильно!`,`Да!`,`Отлично!`,`Супер!`,`Молодец!`],xQ=[`Почти!`,`Близко!`,`Попробуй ещё!`,`Хорошая попытка!`],SQ=10,CQ=class{menuScreen=wQ(`#menu-screen`);gameScreen=wQ(`#game-screen`);completeScreen=wQ(`#complete-screen`);bgSvg=wQ(`#bg-svg`);objSvg=wQ(`#obj-svg`);morphSvg=wQ(`#morph-svg`);questionCard=wQ(`.card`);qText=wQ(`#q-text`);scoreEl=wQ(`#score-val`);qNumEl=wQ(`#q-num`);feedback=wQ(`#feedback`);nextBtn=wQ(`#next-btn`);answersWrap=wQ(`#answers`);drawWrap=wQ(`#draw-wrap`);drawCheckBtn=wQ(`#draw-check-btn`);drawClearBtn=wQ(`#draw-clear-btn`);finalTxt=wQ(`#final-txt`);timerTrack=wQ(`#timer-track`);timerBar=wQ(`#timer-bar`);mouseTracker=new ta;scrollTransition;timer;questionGen;morpher;drawingPad=null;state={phase:`menu`,score:0,questionIndex:0,totalQuestions:SQ,answered:!1,difficulty:`easy`};currentAnswer=0;morphLoopActive=!1;constructor(){new na(this.bgSvg,this.mouseTracker),new ra,this.scrollTransition=new _a(this.questionCard),this.timer=new ya(this.timerBar),this.questionGen=new y(this.objSvg),this.morpher=new Ca(this.morphSvg),this.buildStars(),pa(wQ(`.title`)),ma(this.morphSvg),ha(wQ(`.scroll-hint`)),new gQ(`GSAP version`,this.objSvg,this.morphSvg),this.bindMenuEvents(),this.bindGameEvents(),this.startMenuMorphLoop()}async startMenuMorphLoop(){for(this.morphLoopActive=!0;this.morphLoopActive;)await this.morpher.morphToRandom(),await EQ(1200)}stopMenuMorphLoop(){this.morphLoopActive=!1}bindMenuEvents(){document.querySelectorAll(`.diff-btn`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.diff;this.startGame(t)}),e.addEventListener(`mouseenter`,()=>{ea.to(e,{scale:1.08,rotation:-2,duration:.2,ease:`back.out(2)`})}),e.addEventListener(`mouseleave`,()=>{ea.to(e,{scale:1,rotation:0,duration:.2,ease:`power2.out`})})})}async startGame(e){this.stopMenuMorphLoop(),e===`drawing`&&await this.ensureRecognizerReady(),this.state={phase:`playing`,score:0,questionIndex:0,totalQuestions:SQ,answered:!1,difficulty:e},this.scrollTransition.enabled=!0,this.menuScreen.classList.add(`hidden`),this.gameScreen.classList.remove(`hidden`),this.completeScreen.classList.add(`hidden`),this.loadQuestion()}async ensureRecognizerReady(){let e=wQ(`#loading-overlay`),t=wQ(`#loading-stage`),n=wQ(`#loading-pct`),r=wQ(`#loading-bar`),i=wQ(`.loading-brain`);e.classList.remove(`hidden`);let a=ea.to(i,{scale:1.06,duration:1,yoyo:!0,repeat:-1,ease:`sine.inOut`,transformOrigin:`center center`});await fQ.initRecognizer((e,i)=>{t.textContent=e,n.textContent=`${Math.round(i)}%`,ea.to(r,{width:`${i}%`,duration:.3,ease:`power2.out`})}),await new Promise(e=>setTimeout(e,400)),a.kill(),ea.set(i,{clearProps:`all`}),e.classList.add(`hidden`)}bindGameEvents(){this.nextBtn.addEventListener(`click`,()=>this.advanceQuestion()),this.scrollTransition.onNext(()=>{this.state.answered&&this.advanceQuestion()}),this.drawCheckBtn.addEventListener(`click`,()=>{this.drawingPad?.recognize()}),this.drawClearBtn.addEventListener(`click`,()=>{this.drawingPad?.clear()})}loadQuestion(){this.state.answered=!1,this.feedback.textContent=``,this.feedback.className=`feedback`,this.nextBtn.style.display=`none`,this.answersWrap.innerHTML=``;let e=v[this.state.difficulty],t=this.questionGen.generate(this.state.questionIndex,this.state.difficulty);this.currentAnswer=t.answer,this.qText.textContent=t.text,this.qNumEl.textContent=String(this.state.questionIndex+1),this.scoreEl.textContent=String(this.state.score),e.drawingMode?this.showDrawingMode():(this.showButtonMode(),this.buildAnswerButtons(t.answer)),e.timeLimit?(this.timerTrack.style.display=`block`,this.timer.start(e.timeLimit,()=>this.onTimeUp())):this.timerTrack.style.display=`none`,this.morpher.morphToRandom(),aa(this.objSvg),ia(this.objSvg)}showDrawingMode(){this.answersWrap.style.display=`none`,this.drawWrap.style.display=`flex`,this.drawingPad&&this.drawingPad.destroy();let e=wQ(`#drawing-pad-container`);e.innerHTML=``,this.drawingPad=new fQ(e),this.drawingPad.setCorrectAnswer(this.currentAnswer),this.drawingPad.onRecognized(({digit:e,score:t,correct:n})=>{if(!this.state.answered)if(n)this.state.answered=!0,this.state.score++,this.scoreEl.textContent=String(this.state.score),this.feedback.textContent=bQ[TQ(0,bQ.length-1)],this.feedback.className=`feedback correct`,va(),this.nextBtn.style.display=`inline-block`;else{let n=Math.round(t*100);this.feedback.textContent=e>=0?`Похоже на ${e} (${n}%) — попробуй ещё!`:`Не разобрать — нарисуй чище!`,this.feedback.className=`feedback wrong`}})}showButtonMode(){this.answersWrap.style.display=`flex`,this.drawWrap.style.display=`none`,this.drawingPad&&=(this.drawingPad.destroy(),null)}buildAnswerButtons(e){b(e).forEach((t,n)=>{let r=document.createElement(`button`);r.className=`ans-btn`,r.dataset.v=String(t);let i=vQ[n],a=yQ[n];r.innerHTML=`<svg viewBox="0 0 90 90" xmlns="http://www.w3.org/2000/svg">
      ...
      </svg>`,r.addEventListener(`mouseenter`,()=>da(r,i)),r.addEventListener(`mouseleave`,()=>fa(r)),r.addEventListener(`click`,()=>this.onAnswer(t,e,r)),this.answersWrap.appendChild(r)})}onAnswer(e,t,n){this.state.answered||(this.state.answered=!0,this.timer.stop(),e===t?(this.state.score++,this.scoreEl.textContent=String(this.state.score),this.feedback.textContent=bQ[TQ(0,bQ.length-1)],this.feedback.className=`feedback correct`,va()):(this.feedback.textContent=xQ[TQ(0,xQ.length-1)],this.feedback.className=`feedback wrong`,this.highlightCorrect(t)),this.nextBtn.style.display=`inline-block`)}onTimeUp(){this.state.answered||(this.state.answered=!0,this.feedback.textContent=`Время вышло!`,this.feedback.className=`feedback wrong`,this.highlightCorrect(this.currentAnswer),this.nextBtn.style.display=`inline-block`)}highlightCorrect(e){document.querySelectorAll(`.ans-btn`).forEach(t=>{if(Number(t.dataset.v)===e){let e=t.querySelector(`rect`),n=t.querySelector(`text`),r=t.querySelector(`svg`);e.setAttribute(`fill`,`#00e676`),e.setAttribute(`stroke`,`#ffffff`),e.setAttribute(`stroke-width`,`5`),n.setAttribute(`fill`,`#004d20`),r.style.filter=`drop-shadow(0 0 10px #00e676) drop-shadow(0 0 20px #00e676)`,sa(r)}})}async advanceQuestion(){if(this.state.questionIndex++,this.state.questionIndex>=SQ){this.showComplete();return}await this.scrollTransition.transition(()=>this.loadQuestion())}showComplete(){this.state.phase=`complete`,this.scrollTransition.enabled=!1,this.timer.hide(),this.showButtonMode(),this.gameScreen.classList.add(`hidden`),this.completeScreen.classList.remove(`hidden`),ga(wQ(`.complete-title`)),this.finalTxt.textContent=`Правильно: ${this.state.score} из ${SQ}!`,wQ(`#restart-btn`).onclick=()=>this.startGame(this.state.difficulty),wQ(`#newgame-btn`).onclick=()=>{this.completeScreen.classList.add(`hidden`),this.menuScreen.classList.remove(`hidden`),this.startMenuMorphLoop()}}buildStars(){let e=`http://www.w3.org/2000/svg`,t=this.bgSvg.querySelector(`#star-g`);for(let n=0;n<90;n++){let n=document.createElementNS(e,`circle`);n.setAttribute(`cx`,String(Math.random()*1e3)),n.setAttribute(`cy`,String(Math.random()*700)),n.setAttribute(`r`,String(Math.random()*2+.4)),n.setAttribute(`fill`,`white`);let r=document.createElementNS(e,`animate`);r.setAttribute(`attributeName`,`opacity`),r.setAttribute(`values`,`1;0.05;1`),r.setAttribute(`dur`,`${(Math.random()*3+1.5).toFixed(1)}s`),r.setAttribute(`begin`,`${(Math.random()*4).toFixed(1)}s`),r.setAttribute(`repeatCount`,`indefinite`),n.appendChild(r),t.appendChild(n)}}};function wQ(e){let t=document.querySelector(e);if(!t)throw Error(`Not found: ${e}`);return t}function TQ(e,t){return Math.floor(Math.random()*(t-e+1))+e}function EQ(e){return new Promise(t=>setTimeout(t,e))}document.addEventListener(`DOMContentLoaded`,()=>{new CQ});